			...
			<article>
				<div className={style.containerResume}>
					...
					<div className={style.photo}>
						<div>
							<div className={style.pixelFront} alt={lang === 'EN' ? 'pixelart selfportrait' : 'retrato en pixelart'}> </div>
							<div className={style.pixelBack} alt={lang === 'EN' ? 'pixelart selfportrait' : 'retrato en pixelart'}> </div>
						</div>
					</div>
						{
							resume.data &&
							resume.data.map( e => {
								if (e.side === 'right') {
									return <Infobox key={e.id} color={'Right'} subtitle={e.subtitle} text={e.text} />		
								} else {
										return false
									} 
							})
